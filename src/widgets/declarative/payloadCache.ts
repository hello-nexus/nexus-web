...
//
// Per-widget (not per-instance): copies of the same widget share upstream
// data, and the cache is the worker's last publish, not instance state.
...
export function saveCachedWorkerPayload(widgetId: string, payload: Record<string, unknown>): void {
  ...
  try {
    ...
    if (serialized.length > MAX_PAYLOAD_BYTES) {
      // Skip caching payloads over the cap so one chatty widget can't blow
