...
export function WhiteboardTouch({ widget }: WidgetProps) {
  ...
  const drawerId = `whiteboard-drawer-${widget.id}`;
  ...
  // A tap that dismisses the drawer must not also ink the board: returning
  // true tells the surface to swallow the gesture.
  const panelOpenRef = useRef(false);
  panelOpenRef.current = panel !== null;
  const handleBoardGesture = useCallback(() => {
  ...
  // The immersive overlay owns a modal-stack entry whose Escape closes the
  // whole board. The drawer pushes its own entry so it is topmost and Escape
  // dismisses only the drawer; a plain window listener loses that race,
  // because the stack listens on document in the bubble phase.
  useEffect(() => {
    if (!panel) return;
    const id = `whiteboard-drawer-${widget.id}`;
    pushModalStackEntry({
      id,
      containerRef: drawerRef,
      trapFocus: false,
      onEscape: () => { closePanel(); return true; },
      onEnter: () => false,
    });
    return () => removeModalStackEntry(id);
  }, [panel, widget.id, closePanel]);
  ...
  return (
    <div className={styles.root}>
      ...
      >
        <WhiteboardSurface
          onGestureStart={handleBoardGesture}
