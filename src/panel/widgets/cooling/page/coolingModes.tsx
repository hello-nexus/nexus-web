...
/**
 * Canonical 5-tab cooling mode set, shared by the desktop CoolingPage, the
 ...
 */
export type CoolingModeKey = 'off' | 'silent' | 'balanced' | 'turbo' | 'custom';

/**
 * Widened from `LucideIcon` so the silent/balanced/turbo modes can render the
 * shared <SignalBarsIcon /> (1/2/3 cellphone-style bars); off and custom keep
 * lucide-react icons.
 */
...
// Custom uses lucide Gauge so it does not compete with the signal-bars set.
...
/** Icon for a curve's wire `preset` flag (silent / balanced / turbo); null for
