...
describe('useDeckInstance - debounced auto-save', () => {
  ...
  it('carries the cols/rows grow across a burst of edits within the debounce window (a later edit recomputes grow as undefined against the already-grown local preset)', async () => {
    const { result } = renderHook(() => useDeckInstance('streamdeck:XL1', 'physical', { cols: 8, rows: 4 }));
    await waitFor(() => expect(result.current.loaded).toBe(true));

    act(() => result.current.target!.updateSlot(0, [], 0, { label: 'first' }));
    act(() => result.current.target!.updateSlot(0, [], 1, { label: 'second' }));
    expect(result.current.preset?.cols).toBe(8);
    expect(result.current.preset?.rows).toBe(4);
